package swing.qr.kiarelemb.component.combination;

import swing.qr.kiarelemb.component.basic.QRLabel;
import swing.qr.kiarelemb.component.basic.QRPanel;
import swing.qr.kiarelemb.theme.QRColorsAndFonts;

import javax.swing.*;
import javax.swing.border.EmptyBorder;
import java.awt.*;

/**
 * @author Kiarelemb QR
 * @program: QR_Swing
 * @description:
 * @create 2022-11-24 13:27
 **/
public class QRStatePanel extends QRPanel {

	protected QRPanel leftPane;
	protected QRPanel rightPane;
	protected QRPanel stateCenterPane;

	/**
	 * 一个状态栏一般情况下分为三部分：左、中、右。其中，“中”又用得很少。
	 */
	public QRStatePanel() {
		super();
		EmptyBorder border = new EmptyBorder(10, 10, 10, 10);
		setBorder(border);
		leftPane = new QRPanel(false);
		rightPane = new QRPanel(false);

		setLayout(new BorderLayout());
		add(leftPane, BorderLayout.WEST);
		add(rightPane, BorderLayout.EAST);
		leftPane.setLayout(new GridLayout());
		rightPane.setLayout(new GridLayout());
	}

	public QRStatePanel(int hgap) {
		this();
		leftPane.setLayout(new GridLayout(1, 0, hgap, 0));
		rightPane.setLayout(new GridLayout(1, 0, hgap, 0));
		stateCenterPane.setLayout(new GridLayout(1, 0, hgap, 0));
	}

	public void centerPanel() {
		if (stateCenterPane == null) {
			stateCenterPane = new QRPanel(false);
			stateCenterPane.setLayout(new GridLayout());
			add(stateCenterPane, BorderLayout.CENTER);
		}
	}

	public QRStatePanel leftAdd(JComponent com) {
		leftPane.add(com);
		return this;
	}

	public QRStatePanel rightAdd(JComponent com) {
		rightPane.add(com);
		return this;
	}

	public QRStatePanel leftAddAndSplit(JComponent com) {
		return leftAdd(com).leftSplit();
	}

	public QRStatePanel rightAddAndSplit(JComponent com) {
		return rightAdd(com).rightSplit();
	}

	public QRStatePanel leftSplit() {
		return split(leftPane);
	}

	public QRStatePanel rightSplit() {
		return split(rightPane);
	}

	private QRStatePanel split(QRPanel panel) {
		panel.add(new SplitLabel());
		return this;
	}

	/**
	 * 一个用来分割的标签，颜色为光标的颜色
	 */
	private static class SplitLabel extends QRLabel {
		public SplitLabel() {
			super(" | ");
		}

		@Override
		public void componentFresh() {
			setForeground(QRColorsAndFonts.CARET_COLOR);
		}
	}
}
