...
public class QRGlobalKeyboardHookListener extends GlobalKeyboardHook {
    ...
    List<QRActionRegister<KeyStroke>> press = new ArrayList<>();
    List<QRActionRegister<KeyStroke>> release = new ArrayList<>();

    public QRGlobalKeyboardHookListener() throws UnsatisfiedLinkError {
        super(false);
        press.add(this::keyPress);
        release.add(this::keyRelease);
            ...
            public void keyPressed(GlobalKeyEvent globalKeyEvent) {
                ...
                if (keyStroke != null) {
                    if (press.size() == 1) {
                        ...
                        List<QRActionRegister<KeyStroke>> list = new ArrayList<>(press);
            ...
            public void keyReleased(GlobalKeyEvent globalKeyEvent) {
                ...
                if (keyStroke != null) {
                    if (release.size() == 1) {
                        ...
                        List<QRActionRegister<KeyStroke>> list = new ArrayList<>(release);
                        list.forEach(ar -> ar.action(keyStroke));
                    }
                }
            }
     ...
     */
    public void addKeyListenerAction(boolean press, QRActionRegister<KeyStroke> ar) {
        if (press) {
            this.press.add(ar);
        } else {
            this.release.add(ar);
