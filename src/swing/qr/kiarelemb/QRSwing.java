...
public final class QRSwing implements Serializable {
	...
	/**
	 * 全局配置文件。
	 * 配置文件的读取与写入均为自定义读取。读取可忽略注释，但不会对键值转义，写入时也不会<br/>
	 * 读取时使用 {@link QRPropertiesUtils#loadProp0(Properties, String)}<br/>
	 * 写入时使用 {@link QRPropertiesUtils#storeProp0(Properties, String)}
	 */
	...
	public static void globalPropSave() {
		QRPropertiesUtils.storeProp0(QRSwing.GLOBAL_PROP, GLOBAL_PROP_PATH);
	}
	...
	public static Properties getDefaultSettingsProp() {
		...
		QRPropertiesUtils.loadProp(prop, url);
		return prop;
	}
	...
	private void loadProp(String propFilePath) {
		...
		QRPropertiesUtils.loadProp0(GLOBAL_PROP, propFilePath);
	}
