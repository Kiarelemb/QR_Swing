...
/**
 * QR Swing 中能力最完整的富文本编辑组件。
 *
 * <p>该类基于 {@link JTextPane}，在 Swing 原生富文本能力之上集成了主题刷新、
 * 自定义光标、撤销重做、文本插入、行列计算、鼠标/键盘/文档事件注册、
 * 文件拖入、选区结束事件以及两种滚动容器接入方式。
 *
 * <p>普通多行文本输入可以继续使用 {@link QRTextArea}；如果需要样式文本、
 * 精确行列信息、行号、内部滚动条或更完整的编辑行为，推荐使用 {@code QRTextPane}。
 *
 * @author Kiarelemb QR
 * @program: QR_Swing
...
public class QRTextPane extends JTextPane implements QRComponentUpdate, QRCaretListenerAdd, QRFocusListenerAdd,
				...
				public void mouseReleased(MouseEvent e) {
					if (this.selected) {
						QRTextPane.this.selectionEndListener.selectionEnd(new QRTextSelectionEndEvent(QRTextPane.this
								, caretNearData, getSelectionStart(), getSelectionEnd(), getSelectedText()));
					}
	...
	/**
	 * 为文本面板创建普通 Swing 滚动面板。
	 *
	 * <p>默认每次滚轮滚动 3 行。该方法和 {@link #addInternalScrollPane()}
	 * 互斥，同一个 {@code QRTextPane} 只能选择一种滚动容器。
	 *
	 * @return 承载当前文本面板的 {@link QRScrollPane}
	 */
	...
	/**
	 * 为文本面板创建普通 Swing 滚动面板，并指定平滑滚动的单次滚动行数。
	 *
	 * @param line 单次滚轮滚动的文本行数
	 * @return 承载当前文本面板的 {@link QRScrollPane}
	 */
	 ...
	 * 为文本面板创建内置滚动面板。
	 *
	 * <p>内置滚动面板会把滚动条绘制在文本组件内部，适合需要悬浮滚动条视觉效果的场景。
	 * 该方法和 {@link #addScrollPane()} 互斥，同一个 {@code QRTextPane} 只能选择一种滚动容器。
	 *
	 * @return 承载当前文本面板的 {@link QRInternalScrollPane}
	 */
	...
	public void scrollBarValueUpdate() {
		...
		getScrollBarData().update(getSize(), getLocation(), getParent().getSize(), !lineWrap);
	}
	 ...
	 * 获取指定文档位置在当前视图中的矩形区域。
	 *
	 * <p>该方法是行号绘制、鼠标坐标转文本位置、行列缓存等逻辑的基础。
	 * 如果 Swing 文本视图尚未完成布局，可能返回 {@code null}。
	 *
	 * @param index 文档字符索引
	 * @return 该索引对应的视图矩形，无法计算时返回 {@code null}
	 */
	...
	/**
	 * 从文本面板中搜索第一次出现的内容，并修改其前景色
	 *
	 * @param text  搜索的内容
	 * @param color 新颜色
	 */
	public void changeTextColor(String text, Color color) {
		if (text == null || text.isEmpty()) {
			return;
		}
		int offset = getText().indexOf(text);
		changeTextColor(offset, text.length(), color);
	}
	 ...
	 * @param color     新颜色
	 */
	public void changeTextColor(int fromIndex, String text, Color color) {
		if (text == null || text.isEmpty()) {
			return;
		}
	...
	/**
	 * 从文本面板中搜索第一次出现内容，并修改其前景色
	 *
	 * @param offset 开始位置
	 * @param length 搜索的长度
	 * @param color  新颜色
	 */
	public void changeTextColor(int offset, int length, Color color) {
		if (color == null) {
			return;
		}
		SimpleAttributeSet attrs = new SimpleAttributeSet();
		StyleConstants.setForeground(attrs, color);
		changeTextStyle(offset, length, attrs, false);
	}
	 ...
	 * @param attrs 新样式
	 */
	public void changeTextStyle(String text, AttributeSet attrs) {
		int offset = getText().indexOf(text);
		int length = text.length();
		changeTextStyle(offset, length, attrs, false);
	}
	 ...
	 * @param attrs  新样式
	 */
	public void changeTextStyle(int offset, int length, AttributeSet attrs) {
		changeTextStyle(offset, length, attrs, false);
	}
	 ...
	 * 设置当前段落的行距。
	 *
	 * <p>该方法调用 {@link StyleConstants#setLineSpacing(MutableAttributeSet, float)}
	 * 并通过 {@link #setParagraphAttributes(AttributeSet, boolean)} 应用到当前段落。
	 * 如果需要对全文生效，调用方应在设置文本后选中或覆盖段落属性应用范围。
	 *
	 ...
	 * 重建文本索引、字符矩形和行信息缓存。
	 *
	 * <p>该方法会遍历当前文档的每个字符，通过 {@link #positionRectangle(int)}
	 * 获取其视图矩形，并更新以下缓存：
	 * <ul>
	 *     <li>{@link #indexesAndRectangles}：文档索引到字符矩形的双向映射。</li>
	 *     <li>{@link #rectanglesOfEachLines}：按视觉行分组后的字符矩形数组。</li>
	 *     <li>{@link #lastWordIndexes}：每个视觉行最后一个字符的文档索引。</li>
	 *     <li>{@link #linePerHeight}：用于鼠标 y 坐标粗定位的行高。</li>
	 * </ul>
	 *
	 * <p>当文本内容、字体、换行宽度或段落行距发生变化后，应重新调用该方法，
	 * 否则鼠标定位、行列查询等依赖缓存的功能可能使用旧布局数据。
	 */
	 ...
	 * 根据鼠标坐标查询对应的文本位置信息。
	 *
	 * <p>该方法依赖 {@link #indexesUpdate()} 生成的行矩形缓存。
	 * 先通过 y 坐标定位视觉行，再在该行内根据 x 坐标查找最接近的字符矩形。
	 *
	 * @param p 鼠标在文本面板坐标系中的位置
	 * @return 鼠标对应的文本位置信息；缓存未初始化或位置无效时返回 {@code null}
	 */
	public final QRMousePointIndexData getMousePointIndexData(final Point p) {
		if (p == null || this.indexesAndRectangles == null || this.rectanglesOfEachLines == null
			|| this.indexesAndRectangles.isEmpty() || this.linePerHeight <= 0) {
			return null;
		}
		if (p.y < 0) {
			return null;
	...
	/**
	 * 在一行字符矩形中查找指定 x 坐标对应的列索引。
	 *
	 * <p>为了减少遍历范围，方法先按三等分粗略判断 x 所在区间，
	 * 再交给 {@link #innerIndex(int, Rectangle[], int, int)} 做局部查找。
	 *
	 * @param lineRectangles 同一视觉行内的字符矩形数组
	 * @param x              鼠标或查询点的 x 坐标
	 * @return 对应的列索引，找不到时返回 {@code -1}
	 */
	public final int rowIndex(Rectangle[] lineRectangles, int x) {
		// 计算行矩形的数量
		if (lineRectangles == null || lineRectangles.length == 0) {
			return -1;
		}
		final int lineRectanglesLength = lineRectangles.length;
		if (lineRectanglesLength == 1) {
			return x >= lineRectangles[0].x ? 0 : -1;
		}
		// 计算行矩形数量的三分之一和三分之二位置
	...
	/**
	 * 在指定范围内查找最接近给定 x 坐标的字符矩形索引。
	 *
	 * <p>该方法用于鼠标点击时的列定位。它会把相邻字符矩形之间的边界视为分割线，
	 * 返回更靠近点击位置的字符索引。
	 *
	 * @param value      查询的 x 坐标
	 * @param xes        字符矩形数组
	 * @param startIndex 查找起始索引，包含
	 * @param endIndex   查找结束索引，不包含
	 * @return 匹配到的字符矩形索引，找不到时返回 {@code -1}
	 */
	public final int innerIndex(int value, Rectangle[] xes, int startIndex, int endIndex) {
		...
		endIndex = Math.min(xes.length, endIndex);
		if (startIndex >= endIndex) {
			return -1;
		}
		if (endIndex - startIndex == 1) {
			return value >= xes[startIndex].x ? startIndex : -1;
		}
	 ...
	 * 构造鼠标位置信息对象。
	 *
	 * <p>子类可以重写该方法，在基础的文本索引、行号和列号之外附加更多业务数据。
	 *
	 * @param mousePoint     鼠标在文本面板坐标系中的位置
	 * @param lineRectangles 鼠标所在视觉行的字符矩形数组
	 * @param lineIndex      鼠标所在视觉行索引，从 {@code 0} 开始
	 * @param rowIndex       鼠标所在列索引，从 {@code 0} 开始
	 * @return 鼠标位置信息
	 */
	...
	/**
	 * 设置文本内容，并在使用内置滚动面板时主动刷新滚动条状态。
	 *
	 * <p>{@link JTextPane#setText(String)} 会触发布局和文档更新，但大量文本在同一事件周期内
	 * 设置时，内置滚动条可能早于文本视图完成布局而刷新。这里在设置文本后显式请求
	 * {@link QRInternalScrollPane#refreshScrollBar()}，保证不可编辑文本面板也能显示正确的滚动条。
	 *
	 * @param t 新文本内容
	 */
	@Override
	public void setText(String t) {
		super.setText(t);
		if (this.internalScrollPane != null) {
			this.internalScrollPane.refreshScrollBar();
		}
	}
