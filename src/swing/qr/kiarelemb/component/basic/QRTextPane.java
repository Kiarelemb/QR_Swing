...
//		setPreferredSize(new Dimension(800, 400));
		...
		if (this.selectionEndListener == null) {
			...
			MouseAdapter adapter = new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					if (this.selected) {
						QRTextPane.this.selectionEndListener.selectionEnd(new QRTextSelectionEndEvent(QRTextPane.this, this.caretNearData, getSelectionStart(), getSelectionEnd(), getSelectedText()));
					}
	...
	public final void changeTextsStyle(int offset, int length, AttributeSet attrs) {
		((DefaultStyledDocument) getDocument()).setCharacterAttributes(offset, length, attrs, false);
	}
	...
	public int[] currentLineAndRow(int index) {
		...
		if (mid == -1) {
			...
			this.lineWords = mid != this.lastWordIndexes.length - 1 ? this.lastWordIndexes[mid + 1] - midIndexValue : textLength() - midIndexValue;
		}
	 ...
	 * @return 鼠标位置信息类，可传多参
	 */
	protected QRMousePointIndexData getMousePointIndexDataFinalProcess(final Point mousePoint, final Rectangle[] lineRectangles, final int lineIndex, final int rowIndex) {
		return new QRMousePointIndexData(mousePoint, lineRectangles[rowIndex], this.indexesAndRectangles.getKey(lineRectangles[rowIndex]), lineIndex + 1, rowIndex + 1);
	}
	//endregion
	...
	public String getText() {
		...
		return System.lineSeparator().length() > 1 ? text.replaceAll(System.lineSeparator(), QRStringUtils.AN_ENTER) : text;
	}
